...
class KeyExpansion(keySize: Int) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val done        = Output(Bool())                  // high when last round key is done
  })

  // === Default I/O assignments ===
  io.keyIn.ready := false.B
  ...
  val totalRoundKeys = 4 * (Nr + 1)                 // Total amount of round keys/words
